<template>
  <div class="product-processing-page">
    ...
    <el-table :data="currentPageData" style="width: 100%;">
      ...
      <el-table-column prop="outputcount" label="产出数量" />
    ...
    <el-dialog v-model="dialogVisible" title="产品加工" width="60%" @close="resetForm">
      <el-form :model="formdd" ref="form" label-width="100px">
        ...
        <el-form-item label="产品名称">
          <el-input v-model="formdd.productname" placeholder="请输入产品名称" style="width: 220px;" />
        </el-form-item>
...
const filteredData = ref<Product[]>([]); // 存储经过筛选的数据
...
onMounted(async () => {
  try {
    const response = await api.post('/products/getAll'); // 假设接口返回产品列表
    products.value = response.data;
    filteredData.value = products.value; // 初始化时显示所有数据
  } catch (error) {
